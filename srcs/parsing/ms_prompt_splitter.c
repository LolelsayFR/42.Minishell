...
/*   Updated: 2025/04/07 14:41:50 by emaillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	prompt_split_loop(t_ms_data *data, int i[4], char **result)
{
	...
	while (data->prompt[i[0]])
	{
		if (data->prompt[i[0]] == '"' && quote % 2 == 0)
			...
			quote++;
		else if ((ft_strchr("|<>", data->prompt[i[0]])
				&& (i[0] == 0 || !ft_strchr("<>|", data->prompt[i[0] - 1])
					|| (i[0] > 0 && ft_isspace(data->prompt[i[0] - 1]))))
			&& (quote + d_quote) % 2 == 0)
		{
		...
		i[1]++;
		i[0]++;
		while (ft_isspace(data->prompt[i[0]]) && (d_quote + quote) % 2 == 0)
			i[0]++;
	}
	ft_strncat(&result[i[2]], data->prompt + i[3], i[1]);
}
...
char	**prompt_split(t_ms_data *data)
{
	...
	prompt_split_loop(data, ft_int_aray(0, 4), result);
	return (result);
}
